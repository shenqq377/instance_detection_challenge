...
# -------------------------------------------------------------
# Title: centerCrop.py
# Author: Qianqian Shen
# Date: 2023-03-08
# Description: This is a script for convert annotation format.
# -------------------------------------------------------------
...
def readXML(xml_file):
    ...
    info['dataname'] = []
    info['filename'] = []
    info['width'] = 1024         # 8192
    info['height'] = 768         # 6144
    info['depth'] = 1
    ...
    for i_img, img_file in enumerate(img_list):
        ...
        if flag == 'test':
            anno_path = os.path.join(root_path, 'annotations',
                                     file_name.replace('jpg', 'xml'))  # .xml files for RealScenes
        else:
            anno_path = os.path.join(root_path, 'annotations',
...
if __name__ == '__main__':
    ...
    root_path = "/root/database/Scenes"

    level = 'all'  # 'all', 'hard', 'easy'
    factor = 1 # downsampling x1, x2, x4, x8 for fast computation
    if level == 'all':
        test_path = root_path + "/test_" + str(factor)      
    else:
        test_path = root_path + "/test_" + str(factor) + '_' + level
    ...
    for i, file_path in enumerate(zip(image_paths, anno_paths)):
        file_name = '.'.join(file_path[0].split('.')[0].split('/')[-2:])
        ...
        shutil.copyfile(file_path[0], os.path.join(test_path, 'images', str(i).zfill(3) + '.' + file_name + img_extend)) # add index to make sure image id consistent
        shutil.copyfile(file_path[1], os.path.join(test_path, 'annotations', str(i).zfill(3) + '.' + file_name + anno_extend)) # add index to make sure image id consistent
    
    if level == 'all':
        getCOCOjson(os.path.join(test_path),
                    os.path.join(test_path, "instances_test.json"),
                    factor=1/ factor, flag='test')
    else:
        getCOCOjson(os.path.join(test_path),
                    os.path.join(test_path, "instances_test_" + level + ".json"),
                    factor=1/factor, flag='test') 